...
# Exact Ctrl+F strings to locate each supporting figure in the linked source
SOURCE_HINT = {
    # WACC
    "wacc_rf": '"DGS10" → read top table row (latest date); Sep 2, 2026 ≈ 4.28% → model 4.3%',
    "wacc_erp": '"Implied ERP (FCFE)" → Jan-2026 row ≈ 4.23%; model uses 6.0%',
    "wacc_beta": '"Beta (5Y Monthly)" → Yahoo shows ≈ 0.86; model 0.95',
    "wacc_kd": '"no outstanding borrowings" or "revolving credit facility" → no funded term debt',
    "wacc_tax": '"United States" → Damodaran effective tax rate ~25%; model 27%',
    "wacc_we": '"Cash and cash equivalents" → $1,807,202 | "Long-term debt" → $0',
    "wacc_wd": '"Long-term debt" → none outstanding; debt weight 0%',
    # Scenarios
    "sc_g1": '"full year 2026" → "net revenue" → "decrease of 5% to 7%" | model −6.1% midpoint',
    ...
    "sc_capex_pct": '"Capital expenditures" → FY25 $(680,802); ÷ "Net revenue" = 6.1%; model 5.0%',
    "sc_nwc_pct": '"Inventories" $1,700,753 | "Accounts payable" $331,421 | "Accrued liabilities" $662,982',
    # DCF / comps
    "dcf_exitm": '"Enterprise Value/EBITDA" on StockAnalysis; model terminal exit 8.0x FY2030E EBITDA',
    "comps_nke": '"Enterprise Value/EBITDA" → NKE forward multiple ≈ 18x (illustrative peer)',
    "comps_deck": '"Enterprise Value/EBITDA" → DECK forward multiple ≈ 15x',
    "comps_onon": '"Enterprise Value/EBITDA" → ONON forward multiple ≈ 25x',
    "comps_ads": '"Enterprise Value/EBITDA" → ADDYY forward multiple ≈ 12x',
    "comps_vfc": '"Enterprise Value/EBITDA" → VFC forward multiple ≈ 10x',
    "comps_ff_ev_lo": '"Enterprise Value/EBITDA" → LULU current trough; bear terminal 6.5x',
    "comps_ff_ev_hi": '"Enterprise Value/EBITDA" → bull terminal 9.5x on FY2030E EBITDA',
    "comps_ff_pe_lo": '"Forward P/E" → LULU on depressed FY26E EPS; low case 10.0x',
    "comps_ff_pe_hi": '"Forward P/E" → NKE peer benchmark; high case 18.0x on FY26E EPS',
    "sens_axes": 'Scenarios tab → "WACC" row & "Terminal growth" row (base inputs)',
    "sens_wacc": '"Implied ERP (FCFE)" → anchors WACC range 9.0%–11.0%',
    "sens_g": '"GDPC1" → bounds terminal-g sensitivity 1.5%–3.0%',
    # 3-statement
    "3s_rev_growth": '"full year 2026" → "net revenue" → "5% to 7%" decrease; FY27–30 = projection',
    "3s_gm": '"Gross profit" $6,284,132 ÷ "Net revenue" $11,102,600 = 56.6% FY25',
    "3s_sga_pct": '"Selling, general and administrative expenses" $4,066,556 ÷ revenue = 36.7%',
    "3s_other_opex": '"Amortization of intangible assets" → FY25 $6,961 (≈$7M run-rate)',
    "3s_other_inc": '"Other income (expense), net" → FY25 $28,352 (interest income on cash)',
    "3s_tax_rate": '"Provision for income taxes" $659,784 ÷ "Income before income taxes" $2,238,967 = 29.5%',
    "3s_da_pct": '"Depreciation and amortization" $496,228 ÷ "Net revenue" $11,102,600 = 4.5%',
    "3s_capex_pct": '"Capital expenditures" $(680,802) ÷ "Net revenue" = 6.1%; model fades to 5.0%',
    "3s_sbc": '"Stock-based compensation" → FY25 $62,203; model $70,000',
    "3s_inv_pct": '"Inventories" $1,700,753 ÷ "Cost of goods sold" $4,818,468 = 35.3%',
    "3s_ap_pct": '"Accounts payable" $331,421 ÷ "Cost of goods sold" $4,818,468 = 6.9%',
    "3s_accr_pct": '"Accrued liabilities" $662,982 ÷ "Net revenue" $11,102,600 = 6.0%',
    "3s_oca_pct": '"Other current assets" → derive from current assets less cash, A/R, inventory',
    "3s_rou_pct": '"Operating lease right-of-use assets" $1,630,181 ÷ revenue = 14.7%',
    "3s_onca_pct": '"Other non-current assets" → total non-current assets less PP&E, ROU, goodwill',
    "3s_olc_pct": '"Current portion of operating lease liabilities" $298,724 ÷ revenue = 2.7%',
    "3s_olnc_pct": '"Operating lease liabilities" $1,499,717 (non-current) ÷ revenue = 13.5%',
    "3s_ocl_pct": '"Other current liabilities" → gift cards, deferred revenue (ex-lease) ÷ revenue',
    "3s_oncl_pct": '"Other non-current liabilities" → minor long-term accruals ÷ revenue',
    "3s_buyback": '"Repurchase of common stock" → FY25 $(1,178,349); model $(500,000)/yr',
    "3s_rep_price": '"Repurchase of common stock" footnote → avg price paid per share',
}

COVER_HINTS = {
    "edgar_xbrl": '"10-K" → FY2025 accession 0001397187-26-000020',
    "filing_fy2025": '"CONSOLIDATED STATEMENTS OF OPERATIONS" or "CONSOLIDATED BALANCE SHEETS"',
    "earnings_sep2026": '"full year 2026" | "net revenue" | "diluted earnings per share"',
    "nasdaq_quote": '"LULU" → Last Sale / Previous Close price',
}

REPORTED_HINTS = {
    "10k": '"CONSOLIDATED STATEMENTS OF OPERATIONS" → locate line item below',
    "10k_is": '"Net revenue" → FY2025 $11,102,600 ($000)',
    "10k_ebit": '"Income from operations" → FY2025 $2,210,615 ($000)',
    "10k_bs": '"Cash and cash equivalents" → FY2025 $1,807,202 ($000)',
    "10k_debt": '"Long-term debt" → $0 (no funded debt)',
    "10k_cf": '"Depreciation and amortization" → FY25 $496,228 | "Capital expenditures" $(680,802)',
    "10k_shares": '"Diluted weighted-average number of common shares outstanding" → 119,068 (000)',
    "10k_ebitda": '"Income from operations" $2,210,615 + "Depreciation and amortization" $496,228',
    "earnings_rev": '"full year 2026" → "net revenue" → "$10.35 billion to $10.50 billion"',
    "earnings_eps": '"diluted earnings per share" → "range of $9.48 to $9.73"; model midpoint $9.61',
    "nasdaq": '"Last Sale" or "Previous Close" → LULU share price',
}
