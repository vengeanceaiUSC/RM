...
VAL = {
    ...
    "terminal_value": (
        "B46",
        f"=G36*(1+{SCN_BASE}10)/({SCN_BASE}9-{SCN_BASE}10)",
    ),
    "exit_multiple": ("B47", "=B46/B45"),
    "tv_memo": (
        "B48",
        f"=G36/B45*(1+{SCN_BASE}10)/({SCN_BASE}9-{SCN_BASE}10)",
    ),
    "pv_terminal": ("B49", f"=B46/(1+{SCN_BASE}9)^G37"),
...
def fix(path: Path = TARGET) -> dict:
    ...
    for col in ("C", "D", "E", "F", "G"):
        dcf[f"{col}38"] = f"=1/(1+{SCN_BASE}9)^{col}37"
